-- ================================================
-- FIX STATS DATE ARITHMETIC BUG
-- Generated: 2025-09-29
-- Purpose: Fix date_part(unknown, integer) error in get_stats_data
-- Issue: DATE_PART was being called on integer result of date-date subtraction
-- Solution: Use direct date arithmetic with proper NULL/negative handling
-- ================================================

-- Fix the get_stats_data function with correct date arithmetic
...
BEGIN
  ...
  -- Calculate training consistency (timezone-corrected, fixed date arithmetic)
  training_stats AS (
    SELECT
      GREATEST(
        COALESCE(((now() AT TIME ZONE p_tz)::date - fw.start_date::date) + 1, 0),
        0
      ) AS total_days,
      COALESCE(COUNT(DISTINCT ws.workout_date::date), 0) AS completed_days
...
COMMENT ON FUNCTION public.get_stats_data(text) IS 'Get all stats data for stats page with timezone support - FIXED: removed DATE_PART, uses direct date arithmetic';